...
function run_inversion(prefix, referencefile, perturb_density;
                       method, εs=exp10.(0:-0.25:-3), δ=1e-2, kwargs...)
    if mpi_master()
        println()
        DFTK.versioninfo()
        println()
    ...
    kwargs_model = (; scfres.basis.model.temperature,
                      ...
                      pseudopotentials = [atom.psp for atom in scfres.basis.model.atoms])
    ...
    if mpi_master()
        println("‖ρref - ρorig‖H-1 = ", referror_ρ_hm1)
    ...
    res = kohn_sham_inversion(system, ρref_generator; kwargs_model, kwargs_basis,
                              scfres.ψ, method, εs, δ, vref, kwargs...)

    # Remove mean from potentials to be comparable
    ...
    if method isa InversionVxcMod # Store method parameters
        extra_data["inversion_method_params"] = Dict(
            "α"     => method.α,
            "β"     => method.β,
            "γ"     => method.γ,
        )
