...
function testsystem_path(basis; tol=1e-6)
    ...
    if all(in([:Na, :Cl]),  getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        δ, index = findmin(norm, r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:K, :Cl]), getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        δ, index = findmin(norm, r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:Ga, :As]), getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        @assert basis.model.positions[2] == ones(3)/4
        δ, index = findmin(r -> norm(r - ones(3)/8), r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:Ca, :F]), getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        δ, index = findmin(norm, r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:Mg, :O]), getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        δ, index = findmin(norm, r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:Si]), getfield.(basis.model.atoms, :species))
        # Find point sitting between two bonds
        @assert basis.model.positions[1] == zeros(3)
        @assert basis.model.positions[2] == ones(3)/4
        δ, index = findmin(r -> norm(r - ones(3)/8), r_vectors(basis))
        @assert abs(δ) < tol
    elseif all(in([:Cu]), getfield.(basis.model.atoms, :species))
        # Find bottom-left corner
        @assert basis.model.positions[1] == zeros(3)
        δ, index = findmin(norm, r_vectors(basis))
        @assert abs(δ) < tol
    ...
    atom_symbols = [only(getfield(basis.model.atoms[i_atom], :species)
...
function extract_paths_from_reference(filename)
    ...
    data["vks"] = erp(InversionVks(), scfres.ham, scfres.ρ)[path.indices]
    data["vfa"] = erp(InversionVfa(), scfres.ham, scfres.ρ)[path.indices]
...
function main()
    ...
    files_to_process = filter(readdir("results"; join=true)) do fn
